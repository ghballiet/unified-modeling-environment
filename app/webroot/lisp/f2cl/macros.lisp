...
(declaim (inline aimag conjg dimag))
(defun aimag (c)
  (imagpart c))
(defun dimag (c)
  (imagpart c))
...
(defun lun->stream (lun &optional readp)
  (let ((stream (gethash lun *lun-hash*)))
	...
	(cond ((integerp lun)
	      ...
	      ;; since the type of this function is declaim'd need to
	      ;; ensure all possible return values are of declared type,
	      ;; or error. SBCL compiler will warn otherwise.
	      (T (error "lun->stream arg, lun, value not a key to ~
                         *lun-hash* nor an interger nor a string. LUN : ~A" lun))
	      ))))
...
#-gcl
(declaim (ftype (function (t &optional boolean) stream) lun->stream))

(defvar *NO-PE-717-DIAGNOSTIC-OUTPUT* T)

(defmacro fformat (dest-lun format-cilist &rest args)
  (let ((stream (gensym)))
    `(unless *NO-PE-717-DIAGNOSTIC-OUTPUT*
...
;;; end of macros.l
;;;
;;; $Id: macros.l,v 1.71 2006/05/02 22:12:02 rtoy Exp $
;;; $Log: macros.l,v $
